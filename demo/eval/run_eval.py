...
def output_paths(tag: str | None) -> tuple[Path, Path]:
    ...
    return ROOT / "eval" / f"eval_results_{tag}.json", ROOT / "eval" / f"eval_report_{tag}.md"
...
def score_case(case: dict, result: dict, elapsed_s: float) -> dict:
    ...
    route = next((ev for ev in trace if ev.get("type") == "route"), None) or {}
    return {
        "id": case["id"],
        ...
        "route_candidates": [c.get("id") for c in (route.get("candidates") or []) if c.get("id")],
        "error": error,
        "answer": answer,
    }
...
def main() -> None:
    ...
    parser.add_argument("--tag", default="", help="Write eval_report_{tag}.md instead of eval_report.md")
    args = parser.parse_args()
    tag = (args.tag or "").strip() or None
    out_json, out_md = output_paths(tag)
    ...
    lines = [
        ...
        f"- 用例数：{len(rows)}",
        f"- 自动判定分布：{counts}",
        f"- 中位轮次：{_median(rounds)}（含作答轮，不含目录路由）",
        f"- 中位耗时：{_median(elapsed_vals)}s（含目录路由）",
        ...
        "速度说明：`轮次`含最终作答轮、不含路由轮；`并行`为一轮内同时发出的工具数最大值。",
        "",
        "| ID | 分桶 | IBox | DEMO | 条款命中 | 轮次 | 耗时s | 并行 | 路由方向 | 问题 |",
        "|---|---|---|---|---|---|---|---|---|---|",
    ]
    for r in rows:
        q = r["question"].replace("|", "\\|")
        hit = ",".join(r["clause_hit"]) or "-"
        direction = (r.get("route_direction") or "-").replace("|", "\\|")
        lines.append(
            f"| {r['id']} | {r['bucket']} | {r['ibox_score'] or '-'} / {r['ibox_verdict'] or '-'} "
            f"| {r['verdict']} | {hit} | {r.get('rounds') or '-'} | {r.get('elapsed_s') or '-'} "
            f"| {r.get('max_parallel') or 0} | {direction} | {q} |"
        )
    lines += ["", "## 逐条答案", ""]
    for r in rows:
        cands = ",".join(r.get("route_candidates") or []) or "-"
        lines += [
            f"### {r['id']} {r['question']}",
            "",
            f"- gold: {r['gold_clause_ids']}",
            f"- read: {r['read_clause_ids']}",
            f"- route: {r.get('route_direction') or '-'} | {cands}",
